...
from src.config.constants import (
    MAX_SCHEMA_RESOLUTION_DEPTH,
    MAX_SCHEMA_PROPERTY_DEPTH,
    MAX_CATEGORY_TOOLS,
    MAX_ENDPOINTS_IN_RESPONSE,
)
from src.utils.helpers import parse_host_url, calculate_time_range, build_api_url
...
try:
    from src.logs.croit_log_tools import (
        handle_log_search,
        handle_log_check,
        LOG_SEARCH_TOOLS,
    )
...
try:
    ...
    class TokenOptimizer:
        ...
        @classmethod
        def add_optimization_hints(cls, tool_description, endpoint_path):
...
Many endpoints offer pagination. When available, use it to refine the query."""
        ...
        async def call_tool_handler(
            ...
            try:
                ...
                # Handle argument/data validation errors
                raise RuntimeError(f"Invalid tool arguments: {str(e)}")
                ...
                logger.error(
                    f"Unexpected error in tool handler: {type(e).__name__}: {e}"
                )
                raise RuntimeError(f"Tool execution failed: {str(e)}")

    def _load_config(self) -> None:
        """Load croit API configuration from environment variables."""
        self.host = os.environ.get("CROIT_HOST")
        self.api_token = os.environ.get("CROIT_API_TOKEN")

        if not self.host or not self.api_token:
            raise RuntimeError(
                "Missing required environment variables: CROIT_HOST and CROIT_API_TOKEN\n"
                "Set them with:\n"
    ...
    def _load_local_swagger_spec(self) -> None:
    ...
    def _extract_response_fields(self, endpoint: Dict) -> Dict[str, Any]:
        """
        Extract available fields from endpoint response schema.

        Returns:
            Dict with 'fields' (list of field names) and 'field_descriptions' (dict of field: description)
        """
        try:
            ...
            if "$ref" in response_schema:
                response_schema = self._resolve_reference_schema(
                    response_schema["$ref"]
                )
            ...
            if response_schema.get("type") == "array":
                ...
                if "$ref" in items_schema:
                    items_schema = self._resolve_reference_schema(items_schema["$ref"])
                if "properties" in items_schema:
                    for field, props in items_schema["properties"].items():
                        fields.append(field)
                        field_descriptions[field] = props.get("description", "")

            # Pattern 2: Pagination wrapper or oneOf
            elif "oneOf" in response_schema:
                # Try to find array schema in oneOf
                for option in response_schema["oneOf"]:
                    if option.get("type") == "array":
                        items_schema = option.get("items", {})
                        if "$ref" in items_schema:
                            items_schema = self._resolve_reference_schema(
                                items_schema["$ref"]
                            )
                        if "properties" in items_schema:
                            for field, props in items_schema["properties"].items():
            ...
            # Pattern 3: Object with 'data' field
            elif (
                "properties" in response_schema
                and "data" in response_schema["properties"]
            ):
            ...
            return {"fields": fields, "field_descriptions": field_descriptions}

        except (KeyError, AttributeError, TypeError) as e:
            # Schema structure doesn't match expected format
            logger.debug(f"Could not extract response fields from schema: {e}")
            return {"fields": [], "field_descriptions": {}}

    def _resolve_swagger_references(self) -> None:
        """
...
- "_contains", field contains the filter value
    ...
    def _analyze_api_structure(self) -> None:
        """
    ...
    def _prepare_hybrid_tools(self) -> None:
        """
        Prepare both API exploration tools and category-specific tools.
        This is the 'hybrid' mode that gives maximum flexibility.
        """
        # Base tools
...
• **Shows available_fields for each GET endpoint** - use this to select fields!
...
• Example: available_fields: ["id", "name", "status", "size", "used"]
• Use these to optimize your API calls: fields=["id", "name"]
...
💡 Token Optimization: Results are cached and filtered for efficiency""",
                inputSchema={
                    "type": "object",
                    "properties": {
                        "resource_type": {
        ...
        # Add search tool for drilling down into large responses
        if TOKEN_OPTIMIZER_AVAILABLE:
            self.mcp_tools.append(
                types.Tool(
                    name="search_last_result",
                    description="""Search and filter the last API response.

When an API call returns a summary (large dataset), use this to drill down:
• Filter by any field (e.g., status="error", name__contains="osd")
• Search without re-fetching data from the cluster
• Get detailed data for specific items

Example filters:
• {"status": "error"} - Exact match
• {"name__contains": "osd"} - Substring search
• {"size__gt": 1000} - Numeric comparison
• {"_filter__text": "search term"} - Full-text search

💡 This saves tokens by letting you explore large datasets incrementally""",
                    inputSchema={
                        "type": "object",
                        "properties": {
                            "response_id": {
                                "type": "string",
                                "description": "Response ID from summary (optional, uses last if omitted)",
                            },
                            "filters": {
                                "type": "object",
    ...
    def _extract_schema_properties(self, schema, params, prefix="", depth=0):
        ...
        if depth > MAX_SCHEMA_PROPERTY_DEPTH:
            return
    ...
    def _prepare_category_tools_only(self) -> None:
        """
    ...
    def _prepare_api_tools(self) -> None:
        """
...
1. **Field Selection** (92% token savings!):
   Use the 'fields' parameter to request only needed fields:
   • For listings: fields=["id", "name", "status"]
   • For details: fields=["id", "name", "status", "size", "used"]
   Example: 50 pools with all fields = ~4,000 tokens
            50 pools with 3 fields = ~300 tokens (92% savings!)

2. **Available Fields**:
   Use list_api_endpoints to see available_fields for each endpoint
   
3. **Best Practices**:
   • Always specify fields for list operations (GET /pools, GET /osds, etc.)
   • Only fetch full objects when you need all details
   • Use id+name for simple listings, add status for health checks
   
Examples:
  // Bad (4,000 tokens for 50 pools):
  call_api_endpoint({endpoint: "/pools", method: "get"})
  
  // Good (300 tokens for 50 pools):
  call_api_endpoint({
    endpoint: "/pools", 
    method: "get",
    fields: ["id", "name", "status"]
                ...
                inputSchema={
                    ...
                    "properties": {
                        ...
                        "fields": {
                            "type": "array",
                            "description": "List of field names to return (for token optimization). Only these fields will be included in the response.",
                            "items": {"type": "string"},
                        },
                    },
                    "required": ["endpoint"],
    ...
    def _add_log_search_tools(self) -> None:
...
• Use these values when constructing start_timestamp/end_timestamp queries"""
    ...
    async def _make_api_call(
        ...
        """
        Helper function to make actual API call.
        This function is async, make sure to call it with await before returning result.

        Args:
            url: API endpoint URL
            method: HTTP method
            kwargs: Request kwargs (headers, params, etc.)
            filters: grep-like filters to apply
            requested_fields: List of fields to include in response (for token optimization)
        """
        # Check cache first for GET requests
        if TOKEN_OPTIMIZER_AVAILABLE and method.upper() == "GET":
            params = kwargs.get("params", {})
            cached_result = get_cached_response(url, method, params)
            if cached_result:
        ...
        if TokenOptimizer.should_optimize(url, method):
            ...
            if TOKEN_OPTIMIZER_AVAILABLE:
                url, params = TokenOptimizer.apply_smart_prefilter(url, params)
        ...
        try:
            async with self.session.request(method.upper(), url, **kwargs) as resp:
                ...
                try:
                    ...
                    logger.debug(f"Non-JSON response from {url}: {e}")
                ...
                if (
                    resp.status >= 200
                    and resp.status < 300
                    and TOKEN_OPTIMIZER_AVAILABLE
                ):
                    response_data = optimize_api_response(
                        url=url,
                        method=method,
                ...
                if (
                    ...
                    cache_response(url, method, response_data, kwargs.get("params"))
            ...
            schema_response = {"code": 503, "error": f"Connection failed: {str(e)}"}
            return schema_response
        except asyncio.TimeoutError:
            # Request timeout
            logger.error(f"Request timeout for {method} {url}")
            schema_response = {"code": 504, "error": "Request timeout"}
            return schema_response
        except json.JSONDecodeError as e:
            # Should be caught earlier, but safety net
            logger.error(f"JSON decode error: {e}")
            schema_response = {"code": 500, "error": f"Invalid JSON response: {str(e)}"}
            return schema_response
        except Exception as e:
            # Unexpected errors
            logger.error(f"Unexpected error in API request: {type(e).__name__}: {e}")
    ...
    async def handle_api_call_tool(
        ...
        # Handle search_last_result tool
        if TOKEN_OPTIMIZER_AVAILABLE and name == "search_last_result":
            response_id = arguments.get("response_id")
            filters = arguments.get("filters")
            limit = arguments.get("limit", 100)
            return search_last_result(response_id, filters, limit)
        ...
        if not endpoint.startswith("/"):
            endpoint = "/" + endpoint
        url = build_api_url(f"{self.host}/api", endpoint)
        ...
        # Extract requested fields for field projection
        requested_fields = arguments.get("fields")

        return await self._make_api_call(
            url=url, method=method, kwargs=kwargs, requested_fields=requested_fields
        )
    ...
    async def _handle_log_search(self, arguments: Dict) -> dict[str, Any]:
        ...
        # Extract host, port, and protocol from self.host using helper function
        protocol, hostname, port, use_ssl = parse_host_url(self.host)
        ...
        return await handle_log_search(arguments_with_token, hostname, port)
    ...
    async def _handle_log_check(self, arguments: Dict) -> dict[str, Any]:
        ...
        return await handle_log_check(arguments_with_token, hostname, port)
    ...
    def _list_endpoints_filtered(self, arguments: Dict) -> dict[str, Any]:
        ...
        for path, methods in self.api_spec.get("paths", {}).items():
            for method, operation in methods.items():
                ...
                if method.lower() == "get":
                    response_info = self._extract_response_fields(operation)
                    if response_info["fields"]:
                        fields = response_info["fields"]
                        endpoint_data["available_fields"] = fields

                        # Add smart field selection hint
                        if include_hints:
                            # Suggest essential fields based on common patterns
                            essential = []
                            if "id" in fields:
                                essential.append("id")
                            if "name" in fields:
                            ...
                            if "status" in fields:
                                essential.append("status")

                            if essential:
                                endpoint_data["field_hint"] = (
                                    f"💡 For listings, use fields={json.dumps(essential)} "
                                    f"to reduce tokens by ~90%. Available: {len(fields)} fields total."
                                )
                        else:
                            endpoint_data["supports_field_selection"] = True
                ...
                if llm_hints and include_hints:
                    ...
                    # Merge field hint into llm_hints for better visibility
                    if "field_hint" in endpoint_data:
                        if "usage" not in endpoint_data["llm_hints"]:
                            endpoint_data["llm_hints"]["usage"] = []
                        endpoint_data["llm_hints"]["usage"].insert(
                            0, endpoint_data["field_hint"]
                        )
                        del endpoint_data["field_hint"]
